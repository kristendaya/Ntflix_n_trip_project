...
## jpg dpi 100x100, png dpi 72x72
    for filename in filenames:
        with open("../graph/" + filename, "rb") as image_file:
            ...
            result = img_df.to_sql('netflix', con=engine, if_exists='append', index=False)
...
def store_graph(q,STORE_DIRECTORY):
    with engine.connect() as conn:
        result = list(conn.execute(text(f"select * from netflix where filename='{q[0]}' or filename='{q[1]}'")))
...
def getAPI(countries=None,title=None,url = JSON_HOST, period=None, resource=None):
    if resource != None:
        url += "/"+resource
    url+="?"
    if countries != None:
...
async def total(title):
    resource = "countries"
    result = getAPI(title=user_list[int(title)],resource = resource)
    if not result:
...
async def month(req:Period):
    resource = "countries"
    result = getAPI(title=user_list[int(req.title)], period=[req.period_start,req.period_end],resource=resource)
    if not result:
        return {"ok": False, "data": []}
...
async def make_total_graph(req:TotalGraph):
    ...
    with engine.connect() as conn:
        result = list(conn.execute(text(f"select * from netflix where filename='{filename1}' or filename='{filename2}'")))
    print("아아")
    if (len(result)==0):
...
if __name__ == "__main__":
    uvicorn.run("netflix_backend_server:app", host="0.0.0.0", port=8000, reload=True)
