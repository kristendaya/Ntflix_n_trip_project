...
app.get('/netflix', (req, res) => {
    res.sendFile('/N-T_project/front/public/netflix.html')
})
...
app.get('/netflix-total', (req, res) => {
    ...
    Promise.all([axios.get('http://localhost:8000/col-drop'.concat('?collection=total')), axios.get('http://localhost:8000/netflix-total'.concat("?title=", req.query.title)),
    axios.post('http://localhost:8000/total-graph', {
...
app.post('/netflix-month', (req, res) => {
    ...
    // axios 가 보낼때는 json.stringify 해줘야된다. 문서보면 그렇게해야된다고
    // axios then 하면 이 API가 끝난다?
    Promise.all([axios.get('http://localhost:8000/col-drop'.concat('?collection=period')),
    axios.post('http://localhost:8000/netflix-month', {
        title: req.body.title,
        period_start: req.body.start,
        period_end: req.body.end
    }),
    axios.post('http://localhost:8000/month-graph', {
        title: req.body.title,
        countries: ["Argentina", "Australia", "Costa Rica"],
        period_start: req.body.start,
        period_end: req.body.end
